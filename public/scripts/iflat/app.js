(function($){
    ...
    function () {
        "use strict";
        angular.module("app", ["ngAnimate", "ui.bootstrap", "easypiechart", "mgo-angular-wizard", "app.ui.ctrls", "app.ui.services", "app.controllers", "app.directives", "app.form.validation", "app.ui.form.ctrls", "app.ui.form.directives", "app.tables", "app.task", "app.localization", "app.chart.ctrls", "app.chart.directives"])///.config(["$routeProvider",
                ...
                return {
                    ...
                    compile: function (ele) {
                        ...
                        return $lists = ele.find("ul").parent("li"), /*$lists.append('<i class="fa fa-caret-right icon-has-ul"></i>'),*/ $a = $lists.children("a"), $listsRest = ele.children("li").not($lists), $aRest = $listsRest.children("a"), $a.on("click", function (event) {
                            ...
                            var menuMinified = $(this).closest('.minified').size();
                            $lists.not($parent).removeClass("open ativo").find("ul").slideUp();
                            if ( !menuMinified ){
                                $parent.toggleClass("open").find("ul").slideToggle();
                                return event.preventDefault();
                            }
                        }), $aRest.on("click", function () {
                            return $lists.removeClass("open ativo").find("ul").slideUp()
                        })
                    }
                }
            }
                        ...
                        function ($scope, $element, $attrs, $location) {
                            ...
                            return links = $element.find("a"), path = function () {
                                ...
                                return path = "#" + path, angular.forEach(links, function (link) {
                                    var $li, $link, href;
                                    return $link = angular.element(link), $li = $link.parent("li"), href = $link.attr("href"), $li.hasClass("active"), 0 === path.indexOf(href) ? $li.addClass("active") : void 0
                                })
